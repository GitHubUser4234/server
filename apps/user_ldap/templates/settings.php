	...
	<fieldset id="ldapSettings-1">
		<div id="ldapAdvancedAccordion">
			...
			<div>
				...
				<p><label for="ldap_default_ppolicy_dn"><?php p($l->t('Default password policy DN'));?></label><input type="text" id="ldap_default_ppolicy_dn" name="ldap_default_ppolicy_dn" title="<?php p($l->t('The DN of a default password policy that will be used for password expiry handling. Works only when LDAP password changes per user are enabled. Only supported by OpenLDAP. (An empty setting disables password expiry handling.)'));?>" data-default="<?php p($_['ldap_default_ppolicy_dn_default']); ?>" /></p>
			</div>
